...
File: Script.css
 ...
 Version: 1.0
Description:
  This script contains functions for client-side validation, error message handling, and interactive
  features for a web application.
- validate(): Validates forms for adding or editing book information. It checks fields like title, 
  author, genre, rating, and review. Used in both 'add' and 'edit' modules.
- validateSignup(): Validates user registration form fields like email, username, password.
- validateEmail(email): Checks if the email format is correct.
- clearErrorOnChange(): Clears validation error messages dynamically when the user starts typing in an input field.
- previewImage(): Manages the preview of uploaded image files, specifically for book cover images.
- Event listeners are added to handle dynamic error message clearance and form field validation.
*/

// Validates forms for adding or editing book information. It checks fields like title, 
//author, genre, rating, and review. Used in both 'add' and 'edit' modules.
function validate() {
